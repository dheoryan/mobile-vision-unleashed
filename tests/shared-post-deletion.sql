...
create trigger guard before insert or update on delete_test_dm for each row execute function public.enforce_dm_message_edit_fields();
create trigger guard before insert or update on delete_test_tribe for each row execute function public.enforce_tribe_message_edit_fields();
...
insert into delete_test_dm (sender_id, recipient_id, content, shared_post_id, deleted_at, shared_post_deleted)
select sender, '10000000-0000-0000-0000-000000000003', 'My custom caption', '20000000-0000-0000-0000-000000000001', removed, true
...
insert into delete_test_tribe (sender_id, tribe_id, content, shared_post_id, deleted_at, shared_post_deleted)
select sender_id, '30000000-0000-0000-0000-000000000001', content, shared_post_id, deleted_at, true from delete_test_dm;
create temp table delete_test_before as
select 'dm' as kind, id, to_jsonb(m) - 'shared_post_id' - 'shared_post_deleted' as preserved from delete_test_dm m
union all select 'tribe', id, to_jsonb(m) - 'shared_post_id' - 'shared_post_deleted' from delete_test_tribe m;

-- A sender cannot clear or retarget the reference directly.
do $$
declare tbl text; target uuid;
begin
  if exists (select 1 from delete_test_dm where shared_post_deleted)
     or exists (select 1 from delete_test_tribe where shared_post_deleted) then
    raise exception 'Client forged a deleted-post marker on INSERT';
  end if;
...
begin
  ...
  if exists (select 1 from delete_test_dm where not shared_post_deleted)
     or exists (select 1 from delete_test_tribe where not shared_post_deleted) then
    raise exception 'Deleted shared posts lost their durable marker';
  end if;
  if exists (select 1 from delete_test_dm m join delete_test_before b on b.kind = 'dm' and b.id = m.id
             where b.preserved is distinct from to_jsonb(m) - 'shared_post_id' - 'shared_post_deleted')
     or exists (select 1 from delete_test_tribe m join delete_test_before b on b.kind = 'tribe' and b.id = m.id
                where b.preserved is distinct from to_jsonb(m) - 'shared_post_id' - 'shared_post_deleted') then
...
begin
  foreach tbl in array array['delete_test_dm', 'delete_test_tribe'] loop
    begin
      execute format('update %I set shared_post_deleted = false where sender_id = auth.uid() and deleted_at is null', tbl);
      raise exception 'Client cleared a deleted-post marker';
    exception when raise_exception then
      if sqlerrm = 'Client cleared a deleted-post marker' then raise; end if;
    end;
