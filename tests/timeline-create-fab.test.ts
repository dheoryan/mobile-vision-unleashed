...
test("Timeline creation is a safe-area-aware plus-only FAB, not a header action", () => {
  ...
  assert.match(timeline, /<PlusIcon className="h-6 w-6" weight="bold"/);
...
test("Tribe and Wild creation surfaces use distinct audience color systems", () => {
  assert.match(timeline, /tab === "global" && "bg-meutuals-gradient text-white"/);
  assert.match(timeline, /tab === "tribe" \? \{ backgroundColor: tribe\.colorVar \}/);
  assert.match(composer, /effectiveAudience === "all" && "bg-meutuals-gradient text-white"/);
  assert.match(composer, /effectiveAudience === "tribe" \? \{ backgroundColor: tribe\.colorVar \}/);
  assert.match(repostAudience, /allowWild[\s\S]*"bg-meutuals-gradient text-white"/);
  assert.match(timeline, /tab === "global" && "bg-meutuals-gradient text-white"/);
  assert.match(composer, /effectiveAudience === "all" && "bg-meutuals-gradient text-white"/);
});
...
test("brand system tokens stay separate from Tribe and semantic warning colors", () => {
  ...
  assert.match(styles, /--brand-foreground: #ffffff/);
  assert.match(styles, /--primary: var\(--foreground\)/);
  assert.match(styles, /--primary-foreground: var\(--background\)/);
